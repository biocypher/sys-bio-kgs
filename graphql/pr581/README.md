...
| `pr581_sbml_minimal_fixes.graphql` | PR output with the two fixes needed to use it (see header) |
...
| # | Finding | Evidence |
|---|---|---|
| 1 | Edges without `source`/`target` in the BioCypher schema are dropped without a warning; only an unused `...Props` type is generated. `source`/`target` had to be added to `reactant`, `product`, `modifier`, `contained entity` for testing (documented limitation) | `pr581_sbml_no_source_target.graphql` |
...
| 6 | Unknown types, including BioCypher arrays (`str[]`, `string[]`), become `String` | code |
...
With fixes for 2 and 3, all relationship queries match Cypher and relationship
